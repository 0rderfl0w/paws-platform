...
function VisitScheduleCta({ dogName, locale }: { dogName: string; locale: Locale }) {
  const t = getTranslations(locale);
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const mailto = `mailto:${SHELTER_EMAIL}?subject=${encodeURIComponent(`${t.dogProfile.visitEmailSubject} — ${dogName}`)}&body=${encodeURIComponent(body)}`;
  ...
  return (
    <div className="rounded-[2rem] border border-playful-orange/20 bg-white/88 p-5 text-center shadow-pillowy sm:p-6" data-visit-cta>
      <button
        type="button"
        onClick={openModal}
        data-visit-open
        className="squishy playful-focus inline-flex w-full items-center justify-center gap-2 rounded-full bg-playful-orange px-7 py-4 font-playful-display text-base font-extrabold text-white shadow-squish sm:w-auto"
      ...
      >
        ...
        {t.dogProfile.scheduleVisit}
      </button>
      <p className="mx-auto mt-3 max-w-xl text-sm font-bold leading-6 text-playful-muted">{t.dogProfile.scheduleVisitIntro}</p>

      {isOpen && (
        <div
          data-visit-modal
          className="fixed inset-0 z-[95] flex items-center justify-center bg-playful-ink/55 p-4 backdrop-blur-sm"
          role="dialog"
          aria-modal="true"
        ...
        >
          <div className="relative max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border-2 border-playful-line bg-playful-canvas p-5 text-left shadow-pillowy-lg sm:p-7">
            <button
              data-visit-close
              type="button"
              onClick={closeModal}
            ...
            <h2 id="visit-modal-title" className="mt-2 pr-10 font-playful-display text-3xl font-extrabold tracking-[-0.03em] text-playful-orange-dark">
              {t.dogProfile.visitModalTitle}
            </h2>
            <p id="visit-modal-intro" className="mt-3 text-sm font-semibold leading-7 text-playful-muted sm:text-base">
              {t.dogProfile.visitModalIntro}
            </p>

            <form data-visit-form className="mt-6 space-y-4" onSubmit={handleSubmit}>
  ...
  return (
    <div className="relative mx-auto max-w-7xl" data-dog-profile>
      ...
      <section className="relative z-10 grid min-w-0 items-start gap-10 lg:grid-cols-[1.04fr_0.96fr]" aria-labelledby="dog-profile-heading">
        <div className="min-w-0 space-y-5" data-dog-profile-gallery-column>
          <PhotoGallery photos={photos} name={dog.name} locale={locale} adoptedLabel={dog.is_adopted ? t.status.adopted : undefined} />
          {!dog.is_adopted && <VisitScheduleCta dogName={dog.name} locale={locale} />}
        </div>
